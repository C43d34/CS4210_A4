...
# SPECIFICATION: Train an assortment of artificial neural network models to classify fashion items; and evaluate the models for best accuracy 
...
best_cur_model = ""
best_cur_parameters = (0, 0, 0)
best_cur_accuracy = 0
for hidden_layers in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for neurons_per_hidden_L in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for learning_rate in l_rate:                  #looking or the best parameters w.r.t the learning rate
            ...
            model = build_model(hidden_layers, neurons_per_hidden_L, len(class_names), learning_rate)
            ...
            for classification, ground_truth in zip(class_predictions, y_test):
                if (classification == ground_truth):
                    correct_classifications += 1
                total_classifications += 1
            
            accuracy = correct_classifications / total_classifications
            if (accuracy > best_cur_accuracy):
                best_cur_accuracy = accuracy
                best_cur_model = model
                best_cur_parameters = (hidden_layers, neurons_per_hidden_L, learning_rate)


            print("Highest accuracy so far: " + str(best_cur_accuracy))
            print("Parameters: " + "Number of Hidden Layers: " + str(best_cur_parameters[0]) + ",number of neurons: " + str(best_cur_parameters[1]) + ",learning rate: " + str(best_cur_parameters[2]))
            print()
...
print(best_cur_model.summary())
img_file = './model_arch.png'
tf.keras.utils.plot_model(best_cur_model, to_file=img_file, show_shapes=True, show_layer_names=True)

#plotting the learning curves of the best model
pd.DataFrame(best_cur_model.history).plot(figsize=(8, 5))
plt.grid(True)
